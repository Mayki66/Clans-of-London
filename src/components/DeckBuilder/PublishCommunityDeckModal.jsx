...
export default function PublishCommunityDeckModal({
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!title.trim()) {
      alert("Veuillez saisir un titre pour votre deck.");
      return;
    }
    if (deckCards.length === 0) {
      alert("Votre deck est vide. Ajoutez des cartes avant de le publier.");
      return;
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-lg rounded-2xl bg-gradient-to-b from-[#111420] via-[#0b0d14] to-[#08090e] border border-indigo-500/40 p-6 space-y-5 shadow-[0_0_30px_rgba(99,102,241,0.25)]">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">
              Partagez votre deck avec tous les joueurs de Clans of London
            </p>
          </div>
        </div>
        ...
        <div className="p-3 rounded-xl bg-[#141824] border border-white/10 flex items-center justify-between text-xs font-mono">
          <div>
            <span className="text-gray-400 block text-[10px]">COMPOSITION</span>
            <strong className={`${deckCards.length === 15 ? 'text-emerald-400' : 'text-amber-400'}`}>
              {deckCards.length} / 15 cartes
            </strong>
          </div>
          <div>
            <span className="text-gray-400 block text-[10px]">PUISSANCE TOTALE</span>
            <strong className="text-amber-400 font-bold">{totalPower}</strong>
          </div>
          <div>
            <span className="text-gray-400 block text-[10px]">COÛT MOYEN</span>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="Ex: Brujah Rush Blood, Hecata Soul Engine..."
              required
          ...
          <div className="space-y-1">
            <label className="block text-xs font-gothic font-bold text-gray-300">
              Description & Guide Stratégique
            </label>
            <textarea
              rows={3}
              value={strategy}
              onChange={(e) => setStrategy(e.target.value)}
              placeholder="Expliquez la tactique clé (mulligan, tour 1 à 7, combos de cartes)..."
            ...
            >
              {publishedSuccess ? (
                <>
                  ...
                  <span>Deck publié avec succès dans la Communauté !</span>
                </>
              ) : (
                <>
                  <Share2 className="w-4 h-4 text-indigo-300" />
                  <span>Publier dans les Decks Communauté</span>
                </>
              )}
            </button>
          </div>
