        ...
        <div className="md:w-5/12 p-6 flex flex-col items-center justify-center bg-gradient-to-b from-[#141824] to-[#0a0c12] border-b md:border-b-0 md:border-r border-white/10">
          ...
          <div className="flex items-center space-x-2 mt-4">
            ...
            >
              ...
              <span>{isFoil ? (isFrench ? 'Foil Actif' : 'Foil Active') : (isFrench ? 'Prévisu Foil' : 'Preview Foil')}</span>
            </button>
            ...
            >
              ...
              <span>{isFrench ? 'Modifier Image' : 'Edit Image'}</span>
            </button>
          </div>
          ...
          {editingImage && (
            <form onSubmit={handleSaveImage} className="w-full mt-3 p-3 rounded-xl bg-black/70 border border-amber-500/30 space-y-2 animate-fadeIn">
              <label className="block text-[11px] font-mono text-amber-300">
                {isFrench ? "Coller une URL d'image (Wiki / Web) :" : "Paste image URL (Wiki / Web):"}
              </label>
              <input 
                type="url"
              ...
              <div className="flex items-center justify-between pt-1">
                ...
                >
                  {isFrench ? "Remettre le sceau" : "Reset clan seal"}
                </button>
                <button
                  type="submit"
                ...
                >
                  ...
                  <span>{imageSaved ? (isFrench ? 'Enregistré !' : 'Saved!') : (isFrench ? 'Appliquer' : 'Apply')}</span>
                </button>
              </div>
            </form>
          )}
        ...
        <div className="md:w-7/12 p-6 flex flex-col justify-between space-y-6">
            ...
            <div className="border-b border-white/10 pb-3">
              <div className="flex items-start justify-between gap-2">
                ...
                <button
                  ...
                  title={isFrench ? 'Copier le lien direct vers cette carte' : 'Copy direct link to this card'}
                >
                  <Share2 className="w-3.5 h-3.5 text-amber-400" />
                  <span>{copiedShare ? (isFrench ? 'Lien Copié !' : 'Link Copied!') : (isFrench ? 'Partager' : 'Share Card')}</span>
                </button>
              </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-3 rounded-xl bg-[#090b10] border border-red-500/30 text-center">
                <span className="text-[10px] font-mono uppercase text-gray-400 block mb-1">
                  {isFrench ? 'Coût en Sang' : 'Blood Cost'}
                </span>
            ...
            {synergyCards.length > 0 && (
              <div className="space-y-2 pt-2 border-t border-white/10">
                <h4 className="text-xs font-gothic font-bold uppercase tracking-wider text-gray-400">
                  {isFrench ? 'Cartes Synergiques Recommandées' : 'Recommended Card Synergies'}
                </h4>
          ...
          <div className="p-4 rounded-xl bg-[#090b10] border border-white/10 flex items-center justify-between gap-4">
            <div className="flex items-center space-x-2 font-mono">
              ...
              <span className="text-xs text-gray-400">{isFrench ? 'Statut dans votre Deck :' : 'Status in your Deck:'}</span>
            ...
            <div className="flex items-center space-x-2">
              {countInDeck > 0 && (
                ...
                >
                  ...
                  <span>{isFrench ? 'Retirer du Deck' : 'Remove from Deck'}</span>
                </button>
              )}
              ...
              >
                ...
                <span>{countInDeck >= 1 ? (isFrench ? 'Déjà dans le Deck' : 'Already in Deck') : (isFrench ? 'Ajouter au Deck (1/1)' : 'Add to Deck (1/1)')}</span>
              </button>
            </div>
          </div>
